...
- 2026-05-04:**audit-coverage 项目检查体系 P0 落地 4 笔 atomic** — 用户基于 hardening 闭环后期问"项目检查除纯代码漏洞还需要什么",引出 10 类盘点 C1-C10:✅ 2/10(C3 关键约束 + C10 流程制度)+ 🟡 3/10(C1 业务对齐 / C2 架构对齐 / C5 测试覆盖)+ ❌ 5/10 缺位(C4 性能预算 / C6 依赖供应链 / C7 构建发布 / C8 obs / C9 a11y/i18n)。本期 P0 落地 4 笔 atomic 补 5 类缺位:① `91dca81` 5 份新 SOP commands(/perf-check / /deps-audit / /release-check / /a11y-check)+ obs-checker agent + audit-coverage 报告 231 行 ② `eb764d6` PostToolUse `suggest-checks.cjs` advisory hook(4 类规则:依赖文件 / Vue 组件 / 构建配置 / migrations)+ settings.json 注册 ③ `cefc411` ship-task.md 加 commit 后 7 类智能建议段 + code-audit.md Step 0 用 Bash 自取绕开第三方网关在 Win 中文路径 frontmatter 解析失败 + gate-checker.md 加 milestone 切换流程文件回归检查 ④ 本笔 CURRENT sync。**设计哲学**:**三层智能触发**(L1 hook 单文件视角 / L2 ship-task commit 完整性视角 / L3 gate-checker milestone 跨期视角)+ 几乎只读 + 只写 progress/ + 不修业务代码 + 每条 finding 必给修复路径与编号。**下一步**:M1 末跑 `/perf-check --baseline` 作基线;M1 D6+ ringbuffer logger 接入后跑 `obs-checker` 首次基线巡检;P1(M2 起)整体启用 `/deps-audit` + `/a11y-check` + `/release-check` 进入常态。详 `progress/audit-coverage-2026-05-04.md`(C1-C10 实用性裁决 + P0/P1/P2 路线图)。
- 2026-05-04:**Claude vibecoding harness 用户问询研究 + plan §10 P1-3/P0-2 落地** — 用户问"研究该项目下的 claude 的 vibecoding 开发流程,搞清楚命令/约束/信息怎么工作 + 是否每 session 引入 + 对项目开发有无负面影响"。研究输出 plan 文件 12 节(`C:\Users\TXL\.claude\plans\claude-vibecoding-claude-session-fluttering-anchor.md`)— 整体架构 5 类组件 + 2 类自动注入 + 每 session token 估算 + hook/slash command/permission 三类深度分析 + 综合优化路线图 P0/P1/P2。**关键发现**:研究阶段误判 audit-coverage 报告 § 5 的 5 份 SOP 已落地,实际是 in-flight 未 commit;ExitPlanMode 后 `git status` 才暴露 11 个未 commit 文件 + 用户已在 ship-task.md 用更优的"commit 后智能建议"段实现了 plan §10.1 P0-1 同等效果。**用户选 A 路线**先收口 in-flight 工作:P0-1/P0-3 作废(被覆盖 / 职责重叠),保留 P0-2/P1-3。**6 笔 commit 落地**:audit-coverage 4 笔(同上)+ ⑤ `66114b5` plan §10.2 P1-3 `/milestone-gate` 总入口命令(milestone 末打 1 个命令而非 5+1 个;7 步流程串联 perf-check → deps-audit → release-check → a11y-check → code-audit $branch → gate-checker 综合 → 终端汇总;补完 audit-coverage 三层智能触发 L3 闭环)+ ⑥ 本笔 CURRENT sync 含 plan §10.1 P0-2 `settings.local.json` 漂移清理(精算 21 条:A 桶 17 = 1 taskkill PID + 2 一次性 icon.ico curl + 4 stale 端口 1420 curl + 4 临时调试 echo + 1 echo JSON 固定 path + 1 findstr 1420 + 1 一次性分支重命名 + 2 具体 git diff + 1 git add docs/;B 桶 4 = settings.json 完全重复的 cargo check */pnpm tauri */pnpm add */pnpm remove *;Write 整文件覆盖 91 → 69 行,JSON 合法;**.gitignore 第 54 行排除不进 git**,本地 hygiene 直接生效)。**关键学习**:① **研究阶段必先 git status 看工作树状态**,不要信任 CURRENT.md 的 Last commit 字段(本案 CURRENT 写 8960b24 实际 HEAD 已是 649ae5f);② **plan 文件加「已确认实施范围」段** 区分研究 vs 实施(本案 plan §12);③ **slash command 内不能程序化 chain 调其他 slash command**(SlashCommand 不在 Claude 工具集),`milestone-gate` 等"总入口"命令是 main 主导让 Claude 照子 SOP 步骤逐个跑;④ **`settings.local.json` 漂移**是 harness 唯一无自动维护机制处,5 类残留(具体 PID / 一次性 URL / 临时 echo / 重复 / 巨型 inline python),P2 阶段可写 `/perm-lint` 命令固化清理。
