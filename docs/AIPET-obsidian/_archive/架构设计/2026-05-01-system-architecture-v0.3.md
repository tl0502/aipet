...
- 文档版本：v0.3（基于 v0.2 的增量；未变更章节请参阅 v0.2，再上溯 v0.1）
- 创建日期：2026-05-01
- 适用阶段：MVP 实现前（同步 PRD v0.6）
- 关联：
  - `2026-05-01-system-architecture-v0.2.md`（基线）
  - `需求设计/2026-05-01-ai-desktop-pet-prd-v0.6.md`
  - `需求设计/2026-05-01-ai-desktop-pet-flows-v0.6.md`（待升）
  - `角色与人格/2026-05-01-persona-design-v0.2.md`（待升）

## 0. v0.2 → v0.3 变更摘要

| 类别 | 内容 |
|---|---|
| 新增 Rust 服务 | InteractionRouter / VoiceEffectPlayer / WardrobeService / GameEngine（含 LocalGameRunner + LLMGameRunner） / NicknameService |
| 既有服务扩展 | LivingPetService 增加日常时段表（DailySchedule） / MilestoneService 增加 user_anniversaries 规则集 |
| 新增 SQLite 表 | accessories_inventory / wardrobe_decisions / voice_packs / voice_settings / game_sessions / user_anniversaries / nicknames |
| 新增 IPC 命令族 | interaction.* / voice.* / wardrobe.* / game.* / nickname.* / anniversary.* |
| 新增事件 | pet.interaction_*, voice.played, wardrobe.changed, game.* |
| 性能预算追加 | 物理交互 < 100ms、装扮切换 < 500ms、本地游戏 < 50ms/轮 |
| 资源管理 | 内置音效 ≤ 5MB、装扮 ≤ 10MB、总安装包 ≤ 80MB |
...
### 13.1 v0.3 未决项（M0 决策周）
1. 音效包来源（自录 / 采购 / CC0）。
2. 配饰美术管线对 Live2D / Spine 兼容方案。
3. 装扮付费 schema 范式（tier / unlock 字段枚举）。
4. 小游戏 UI 风格（嵌入对话面板 / 独立小窗口 / 桌宠头顶气泡）。
5. LLM 游戏场景白名单（咖啡店老板等）+ 每场景拒答模板（≥ 3 条）。
6. 物理交互每个 hitbox 的反应动作清单（≤ 12 个独立动作 ID）。
7. RAWINPUT 实现可行性（M2 内决断）。
